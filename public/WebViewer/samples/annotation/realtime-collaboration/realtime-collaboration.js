...
WebViewer(
  ...
  docViewer.on('documentLoaded', function() {
    ...
    function onAnnotationCreated(data) {
      ...
      annotManager.importAnnotCommand(data.val().xfdf).then(function(annotations) {
    ...
    annotManager.on('annotationChanged', function(annotations, type, info) {
      // info.imported is true by default for annotations from pdf and annotations added by importAnnotCommand
      if (info.imported) {
        return;
      }

      annotManager.exportAnnotCommand().then(function(xfdf) {
